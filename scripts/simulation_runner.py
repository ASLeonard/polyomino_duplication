...
def runnerCore(G_RATE):
    ...
    default_args=' -N 3 -P 100 -B 20 -X .25 -F 1 -A 1 -V 0 -T 10 -M .00075 -D 100 -J {0} -L {0}'.format(G_RATE)

    generations = {.75:50000}
    for S_c, gen in generations.items():
        print('Running simulation at S: {}'.format(S_c))
        full_args = default_args + ' -Y {} -G {}'.format(S_c, gen)
        
        subprocess.run('../bin/DuplicationEvolution_L64 -E'+full_args,shell=True)
        return
