...
def convertTransitionstoDF(data):
     ...
     for phen,trans in data.transitions.items():
          if phen not in pids:
               continue
          for phen2,counts in trans.items():
               df[origins[phen2]][pids[phen]]=counts
     return df
...
class EvoData(object):
     __slots__ = ('S_star','T','mu','du','ins','de','transitions','N_runs','evo_strs','evo_samps','jumps')
     def __init__(self,S_star,T,mu,du,ins,de,data):
          self.S_star=S_star
          self.T=T
          self.mu=mu
          self.du=du
     ...
     def __repr__(self):
          return r'Data struct for S*={:.3f},T={:.3g},mu={:.2g},du={:.2g}'.format(self.S_star,self.T,self.mu,self.du)
     
def loadPickle(S_star,t,mu,du,ins,de):
     with open('/rscratch/asl47/Pickles/Y{}T{}Mu{}J{}K{}L{}.pkl'.format(S_star,*(float(i) for i in (t,mu,du,ins,de))), 'rb') as f:
          return EvoData(S_star,t,mu,du,ins,de,load(f))
